...
bool MapHash::save(std::string path)const
{
    ...
    for(std::pair<FrameID,FramePtr> it:_frames){
        SPtr<MapFrame> fr=std::dynamic_pointer_cast<MapFrame>(it.second);
        std::string imgFile;
        ...
        std::map<GSLAM::FrameID,SPtr<GSLAM::FrameConnection> > children,parents;
        fr->getChildren(children);
        fr->getParents(parents);
        std::vector<std::pair<GSLAM::FrameID,int> > parentsVec,childrenVec;
        for(std::pair<GSLAM::FrameID,SPtr<GSLAM::FrameConnection> > c:children){
            childrenVec.push_back(std::make_pair(c.first,int(c.second?c.second->matchesNum():-1)));
        }
        for(std::pair<GSLAM::FrameID,SPtr<GSLAM::FrameConnection> > c:parents){
...
bool MapHash::load(std::string path)
{
    ...
    for(int i=0;i<frameNum;i++){
        ...
        SPtr<MapFrame> fr(new MapFrame(id,timestamp,img,imgFile,GSLAM::Camera(camParas),gpsData,imageChannel));
        ...
        for(std::pair<GSLAM::FrameID,int> c:children){
            fr->addChildren(c.first,SPtr<GSLAM::FrameConnection>(new MapFrameConnection(c.second)));
        }
        for(std::pair<GSLAM::FrameID,int> c:parents){
            fr->addParent(c.first,SPtr<GSLAM::FrameConnection>(new MapFrameConnection(c.second)));
        }
...
bool MapHash::saveMap2DFusion(std::string foler2save)const
{
    ...
    SPtr<GSLAM::Estimator> estimator=GSLAM::Estimator::create();
...
bool MapHash::saveTrajectory(std::string file2save)const
{
    ...
    ofs.precision(6);
    for(GSLAM::FramePtr& fr:frames){
        ofs<<GSLAM::Svar::dtos(fr->timestamp(),6)<<setiosflags(ios::fixed)<<setprecision(9)<<" "<<fr->getPose()<<endl;
    }
    return true;
}
...
bool MapHash::saveDense(std::string folder2save)const
{
    ...
    for(auto& pt:points)
    {
        if(!pt) continue;
        ofs<<pt->getPose()<<"\n"
          <<(pi::Point3i)pt->getColor()<<endl;
        std::map<GSLAM::FrameID,size_t> observations;
        pt->getObservations(observations);

        vector<std::pair<pi::Point2i,pi::Point2d> > observes;
        ...
        for(auto& obs:observations)
        {
            ...
            std::pair<int,pi::Point2d> kp(obs.second,pi::Point2d());
            fr->call("GetKeyPoint",&kp);

            observes.push_back(make_pair(pi::Point2i(frameID,obs.second),
                                         kp.second-pi::Point2d(cx,cy)));
...
typedef GSLAM::Map Map;
    extern "C" std::shared_ptr<GSLAM::Map> createHash(){ return SPtr<MapHash>(new MapHash());}\
    class MapHashHash_Register{ \
    public: MapHashHash_Register(){\
    GSLAM::SvarWithType<funcCreateMap>::instance().insert("Hash",createHash);\
}}MapHashHash_instance;
